...
// @Description Readiness probe for Kubernetes (checks PostgreSQL and Redis connectivity)
// @Tags system
// @Accept json
// @Produce json
...
func Readiness(db *postgresql.PostgresDB, redisDB *redis.RedisDB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		dbErr := db.DB.PingContext(ctx)
		redisErr := redisDB.Client.Ping(ctx).Err()
		...
		response := map[string]interface{}{
			...
			"repositories": map[string]interface{}{
				"database": getStatus(dbErr),
				"redis":    getStatus(redisErr),
			},
		}
